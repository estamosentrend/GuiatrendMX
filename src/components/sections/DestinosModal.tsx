...
export default function DestinosModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const apiUrl = open ? `/api/lugares?categoria=Destinos&ciudad=${encodeURIComponent(ciudad?.nombre || '')}&destacados=true` : null;
  
  // Construir la URL de la API para forzar actualización (sin cache)
  const apiUrlForceRefresh = open ? `/api/lugares?categoria=Destinos&ciudad=${encodeURIComponent(ciudad?.nombre || '')}&destacados=true&forceRefresh=true` : null;

  // Usar useSWR para la carga de datos con revalidación manual
  const { data: lugares, isLoading, error, mutate } = useSWR<Lugar[]>(apiUrl, fetcher, {
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto hide-vertical-scrollbar">
              ...
              {lugares && (
                <div className="mb-4 p-2 bg-yellow-100 text-yellow-900 rounded text-xs">
                  <div><b>Debug:</b> Total lugares recibidos: {Array.isArray(lugares) ? lugares.length : 0}</div>
                  <div>Ciudades en datos: [{Array.isArray(lugares) ? lugares.map(l => l.ciudad).join(', ') : 'No disponible'}]</div>
