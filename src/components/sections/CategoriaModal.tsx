...
// Reutilizamos las mismas ciudades del componente DestinosModal
const ciudades = [
  {
    key: 'monterrey',
    nombre: 'Monterrey',
...
// Coordenadas para cada ciudad (reutilizadas del componente DestinosModal)
...
// Componente para mostrar el clima (reutilizado del componente DestinosModal)
  ...
  return (
    <div className="mt-6 mb-4 rounded-2xl glass-card p-4 text-center">
      <h4 className="text-lg font-semibold mb-2 text-[var(--color-text-dark)] dark:text-white flex items-center justify-center gap-2">
        <svg className="w-5 h-5 text-[#3DFFAB] fill-current" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path d="M8 15h8M12 8v7" /></svg>
        Clima actual
      </h4>
...
export default function CategoriaModal({ open, onClose, categoria }: CategoriaModalProps) {
  ...
  const getCategoriaIcon = () => {
    switch (categoria) {
      case 'Gastronomía': return <FiCoffee className="text-primary w-6 h-6" />;
      case 'Hospedaje': return <FiHome className="text-primary w-6 h-6" />;
      case 'Emergencias': return <FiAlertCircle className="text-primary w-6 h-6" />;
      case 'Directorio': return <FiBookOpen className="text-primary w-6 h-6" />;
      case 'Servicios': return <FiTool className="text-primary w-6 h-6" />;
      case 'Inmuebles': return <FiBuilding className="text-primary w-6 h-6" />;
      case 'Promociones': return <FiTag className="text-primary w-6 h-6" />;
      default: return <FiMapPin className="text-primary w-6 h-6" />;
    }
  };
        ...
        <div className="w-full h-full flex items-center justify-center">
          <div className="relative w-full max-w-5xl xl:max-w-6xl mx-auto my-8 mt-24 rounded-3xl shadow-2xl bg-white/70 dark:bg-[#181C20]/60 border border-white/30 dark:border-gray-800/60 backdrop-blur-lg overflow-hidden animate-fade-in">
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto hide-vertical-scrollbar">
              ...
              <div className="mb-6">
                <h3 className="text-lg font-bold mb-4 text-[var(--color-text-dark)] dark:text-white text-center">{categoria} en {ciudad?.nombre}</h3>
                {isLoading ? (
                  ...
                  <div className="text-center text-gray-400 dark:text-gray-500 py-8">No hay {categoria.toLowerCase()} registrados para esta ciudad.</div>
                ) : (
